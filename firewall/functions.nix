...
in {
  ...
  vlanRules = wanIf: privateNets: name: id: ''
    # ${name} (VLAN ${toString id}): Allow internet and own subnet, block other VLANs
    iifname ${lib.custom.vlanIf id} ip daddr ${lib.custom.vlanNet id} ${mkLog "vlan" "accept"}
    iifname ${lib.custom.vlanIf id} ip daddr ${privateNets} limit rate 10/second ${mkLog "vlan" "drop"}
    iifname ${lib.custom.vlanIf id} oifname ${wanIf} ${mkLog "accept" "accept"}
  '';
